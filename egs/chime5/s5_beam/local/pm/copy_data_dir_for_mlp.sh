...
cat $srcdir/utt2spk | awk '{print $1}' | awk -F- '{N=NF-2; print $N}' | sed 's/_'$mic'//g' | awk '{printf "%s_%06d\n",$1,NR;}'  > $destdir/utt_map_1
cat $srcdir/utt2spk | awk '{print $1}' > $destdir/utt_map_2
paste -d" " $destdir/utt_map_2 $destdir/utt_map_1 > $destdir/utt_map
rm $destdir/utt_map_1
rm $destdir/utt_map_2
#cat $srcdir/utt2spk | awk -v p=$temp '{printf("%s %s\n", $1, p);}' > $destdir/utt_map
