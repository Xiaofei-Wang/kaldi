...
if [[ ( $# -lt 3 ) || ( $# -gt 5 ) ]]; then
   echo "usage: steps/nnet3/make_phone_feats_nnet3.sh <input-data-dir> <nnet-dir> <phone-data-dir>[<log-dir> <phone-feat-dir>]"
   echo "e.g.:  steps/nnet3/make_phone_feats_nnet3.sh data/train exp/nnet3 data-phone/train data-phone/train/log phone-feats"
...
if [ $stage -le 1 ]; then
  ...
  nnet3-compute $compute_gpu_opt --apply-exp=${apply_exp} "$modified_bnf_nnet" "$feats" ark:- \| 
