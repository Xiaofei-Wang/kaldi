...
transf_nnet_out_opts="--pdf-to-pseudo-phone=$mdelta_stats_dir/pdf_to_pseudo_phone.txt"
  ...
  utils/phone_post/create_pdf_to_phone_map.sh /export/b03/xwang/mixer6/kaldi/egs/rm/s5/data/lang $nndir/final.mdl $mdelta_stats_dir
...
#    $mdelta_stats_dir/phones.tra >$mdelta_stats_dir/pri
  ...
  $cmd JOB=1:$nj $logdir/compute_m-measure_scores.JOB.log \
    ...
    python utils/multi-stream/pm_utils/compute_m-measure.py \$tmp_dir/post.JOB.scp $bnfeadir/mmeasure_scores.JOB.pklz '&&' \
  ...
  python utils/multi-stream/pm_utils/merge_dicts.py $comb_str $bnfeadir/mmeasure_scores.$name.pklz 2>$logdir/merge_dicts.log || exit 1;
  python utils/multi-stream/pm_utils/dicts2txt.py $bnfeadir/mmeasure_scores.$name.pklz $bnfeadir/mmeasure_scores.$name.txt 2>$logdir/dicts2txt.log || exit 1;
