...
def compute_mse(args):
    ...
    for d, (utt_in, value_in) in enumerate(autoencdoer_feats_in):
        ...
        mse1 = np.sum(mse)
        mse_tol = mse_tol + mse1
        frame_num = frame_num + len(mse)

    mse_tol = mse_tol* 1.0 / frame_num

    logger.info("\"{mse_tol}\" Store the mse dictionary as a pickle object in local disk.".format(mse_tol=mse_tol))
