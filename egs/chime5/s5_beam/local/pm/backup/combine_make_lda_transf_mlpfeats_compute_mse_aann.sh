...
required=$required" $transf_dir/feature_extractor.nnet $transf_dir/final.feature_transform $transf_dir/lda${lda_dim}.mat"
...
ldamat=$transf_dir/lda${lda_dim}.mat
...
$cmd JOB=1:$nj $logdir/make_lda_transf.JOB.log \
  ...
  transform-feats ${ldamat} ark:- \
  ark,scp:\$tmp_dir/lda_transf_$name.JOB.ark,\$tmp_dir/lda_transf_$name.JOB.scp '&&' \
  compute-cmvn-stats --spk2utt=ark:${sdata}/JOB/spk2utt scp:\$tmp_dir/lda_transf_$name.JOB.scp \
  ark,scp:\$tmp_dir/lda_transf_$name.JOB.cmvn.ark,\$tmp_dir/lda_transf_$name.JOB.cmvn.scp '&&' \
  nnet-score-sent --use-gpu=no --objective-function=mse --feature-transform=${aann_dir}/final.feature_transform \
  ${aann_dir}/final.nnet "ark,s,cs:copy-feats scp:\$tmp_dir/lda_transf_$name.JOB.scp ark:- | apply-cmvn --norm-means=true --norm-vars=true --utt2spk=ark:${sdata}/JOB/utt2spk scp:\$tmp_dir/lda_transf_$name.JOB.cmvn.scp ark:- ark:- |" "ark,s,cs:copy-feats scp:\$tmp_dir/lda_transf_$name.JOB.scp ark:- | apply-cmvn --norm-means=true --norm-vars=true --utt2spk=ark:${sdata}/JOB/utt2spk scp:\$tmp_dir/lda_transf_$name.JOB.cmvn.scp ark:- ark:- | nnet-forward ${aann_dir}/final.feature_transform ark:- ark:- | feat-to-post ark:- ark:- |" ark:- \| \
  select-feats 0 ark:- ark,t,scp:${mse_data_dir}/mse.JOB.txt,${mse_data_dir}/mse.JOB.scp || exit 1; 
