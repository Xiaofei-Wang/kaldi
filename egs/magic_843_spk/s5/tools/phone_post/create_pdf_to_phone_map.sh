...
tools/phone_post/create_pseudo_phones.py $lang/phones/roots.txt 2>$logdir/create_pseudo_phones.log >$outdir/pseudo_phones.txt || exit 1;
...
#tools/phone_post/show_transitions_to_sym2int.py $outdir/show_transitions.txt $lang/phones/roots.txt | utils/sym2int.pl -f 2 $outdir/pseudo_phones.txt | sort -n -k 1 | uniq > $outdir/pdf_to_pseudo_phone.txt || exit 1;
