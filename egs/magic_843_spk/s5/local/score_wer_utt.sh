...
if [ $stage -le 1 ]; then
  ...
  if [ -z "$best_lmwt" ]; then
    ...
    [ -e $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_${best_lmwt}.txt ] &&\
    rm $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_${best_lmwt}.txt
    while read -r u; do
        ...
        ark,p:- > $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_${best_lmwt}_utt_$u.txt || exit 1;
        wer_command=`cat $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_${best_lmwt}_utt_${u}.txt |\
        grep "WER" | cut -d " " -f2`
        echo $u $wer_command >> $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_$best_lmwt.txt
        done < $dir/scoring_kaldi/penalty_$best_wip/$best_lmwt.uttid
        rm $dir/scoring_kaldi/wer_utt_penalty_${best_wip}_lmwt_${best_lmwt}_utt_*.txt
  fi
