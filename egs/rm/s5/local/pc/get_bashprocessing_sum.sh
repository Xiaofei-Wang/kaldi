...
for i1 = 1: length(tasks)
  ...
  task=tasks{i1};
%  weight_file=[weight_dir,task,'_sum_comb/weight/stream_weights.scp'];
...
%	    end
    ...
    outdir=['exp/dnn4_pretrain-dbn_dnn/decode_test_sum_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn4_pretrain-dbn_dnn/decode_test_sum_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   
  end

  end

" > local/pc/sum/postcombination_sum_${JOB}.m
