...
mkdir -p local/pc/frame_tdnn_5layertdnn_context_5_5
...
for i1 = 1: length(tasks)
  ...
  task=tasks{i1};
  weight_file=[weight_dir,task,'_frame_tdnn_5layertdnn_context_5_5_comb/weight/stream_weights.scp'];
  ...
  for job=$JOB 
    ...
    outdir=['exp/dnn4_pretrain-dbn_dnn/decode_',task,'_frame_tdnn_5layertdnn_context_5_5_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn4_pretrain-dbn_dnn/decode_',task,'_frame_tdnn_5layertdnn_context_5_5_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   
  end

  end

" > local/pc/frame_tdnn_5layertdnn_context_5_5/postcombination_use_frame_tdnn_5layertdnn_context_5_5_${JOB}.m
