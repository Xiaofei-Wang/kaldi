...
mkdir -p local/pc/frame_tdnn_${nbest}best
...
for i1 = 1: length(tasks)
  ...
  task=tasks{i1};
  weight_file=[weight_dir,task,'_frame_tdnn_${nbest}best_comb/weight/stream_weights.scp'];
  ...
  for job=$JOB 
    ...
    outdir=['exp/dnn4_pretrain-dbn_dnn/decode_',task,'_frame_tdnn_${nbest}best_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn4_pretrain-dbn_dnn/decode_',task,'_frame_tdnn_${nbest}best_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   
  end

  end

" > local/pc/frame_tdnn_${nbest}best/postcombination_use_frame_tdnn_${nbest}best_${JOB}.m
