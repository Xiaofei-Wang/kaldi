...
for i1 = 1: length(tasks)
  ...
  task=tasks{i1};
  weight_file=[weight_dir,task,'_AE4best_comb/weight/stream_weights.scp'];
  ...
  for job=$JOB 
    ...
    outdir=['exp/dnn4_pretrain-dbn_dnn/decode_dev_AE4best_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn4_pretrain-dbn_dnn/decode_dev_AE4best_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   
  end

  end

" > local/pc/AE4best/postcombination_use_AE4best_${JOB}.m
