...
if [ $# != 6 ]; then
   echo "usage: $0 [options] <tgt-data-dir> <src-data-dir> <multi-stream-opts> <lda-dir> <log-dir> <abs-path-to-feat-dir>";
   echo "options: "
...
required="$srcdata/feats.scp $ldadir/feature_extractor.nnet $ldadir/final.feature_transform $ldadir/lda.mat"
...
$cmd JOB=1:$nj $logdir/make_lda_transf.JOB.log \
  ...
  transform-feats ${ldamat} ark:- ark,scp:${bnfeadir}/lda_transf_$name.JOB.ark,${bnfeadir}/lda_transf_$name.JOB.scp || exit 1;
# concatenate the .scp files
(
for ((n=1; n<=nj; n++)); do
  cat $bnfeadir/lda_transf_$name.$n.scp
done
) > $data/feats.scp
...
echo "Succeeded creating lda_transf_mlpfeats features '$data'"
