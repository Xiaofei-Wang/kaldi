...
dir=data/local/test/CH$channel
mkdir -p $dir
mkdir -p $SEGS
...
awk '{meeting=$1; channel=$2; speaker=$3; stime=$4; etime=$5;
 ...
 for(i=6;i<=NF;i++) printf(" %s", $i); printf "\n"}' $SEGS/test.txt | sort | uniq > $dir/text
...
fdir=data/test/CH$channel
mkdir -p $fdir
for f in spk2utt utt2spk wav.scp text segments reco2file_and_channel; do
  cp $dir/$f data/test/CH$channel/$f || exit 1;

done

utils/validate_data_dir.sh --no-feats data/test/CH$channel || exit 1;
