...
%    index_fid=fopen('ppp.txt','w');
    ...
    for j = 1 : num_stream
        if j < 10
            mmeasure_name{j} = ['data-mmeasure/test/CH0',num2str(j),'/data/mmeasure_scores.CH0',num2str(j),'.txt' ];
            score_name{j} = ['exp/dnn4_pretrain-dbn_dnn/decode_test_CH0',num2str(j),'/scoring/1.tra'];
        else   
            mmeasure_name{j} = ['data-mmeasure/test/CH',num2str(j),'/data/mmeasure_scores.CH',num2str(j),'.txt' ];
...
%    mmeasure_v = zeros(length(file_list), num_stream);
    
    %second, compare the mmeasure
    for j = 1 : num_stream
%        [names{j}, mmeasure_v{j}] = textread('mmeasure_scores.SimData_dt_for_1ch_far_room1_A.txt','%s %f ');
        [names{j}, mmeasure_v{j}] = read_mmeasure_file(mmeasure_name{j});
    end
...
%        fprintf(index_fid,'\n');
    ...
    ofdir = 'exp/dnn4_pretrain-dbn_dnn/decode_test_mmeasure_comb/scoring';
    mkdir(ofdir);
    ofname = [ofdir,'/1.tra'];
    ...
    fclose(out_file);
%    fclose(index_fid);
    clear names mmeasure_v best_score file_list score_file
