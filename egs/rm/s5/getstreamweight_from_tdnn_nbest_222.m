...
autoencoder_dir='data-tdnn-autotncoder/';
...
for i1 = 1: length(tasks)
      ...
      for i2 = 1: length(scenes)
        scene=scenes{i2};
        feature_name=[autoencoder_dir,task,'/',scene,'/tdnn_autotncoder_mse.ark'];
  ...
  for nbest = 4 : 9
    ...
    for utt_id = 1 : number_utt
        ...
        % the second paremeter should be the n-best values
        enhanced_phoneme = analyse_phoneme_get_nbest_autoencoder(tmp,nbest);
    ...
    outdir=[weight_dir,task,'_frame_tdnn_',num2str(nbest),'best_comb/weight'];
    mkdir(outdir);
