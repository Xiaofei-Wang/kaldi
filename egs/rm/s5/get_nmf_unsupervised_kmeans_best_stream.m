...
r=40;
    for j = 1 : num_stream
        if j < 10
            nmf_name{j} = ['data-posterior/dev/CH0',num2str(j),'/unsup_nmf_rank',num2str(r),'_kmeans.txt' ];
            score_name{j} = ['exp/dnn4_pretrain-dbn_dnn/decode_dev_CH0',num2str(j),'/scoring/1.tra'];
        else   
            nmf_name{j} = ['data-posterior/dev/CH',num2str(j),'/unsup_nmf_rank',num2str(r),'_kmeans.txt' ];
