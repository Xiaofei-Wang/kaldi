...
clear all;
    ...
    for j = 1 : num_stream
        if j < 10
            autoencoder_name{j} = ['data-mmeasure-mse/dev/CH0',num2str(j),'/autoencoder_scores.txt' ];
            score_name{j} = ['exp/dnn4_pretrain-dbn_dnn/decode_dev_CH0',num2str(j),'/scoring/1.tra'];
        else   
            autoencoder_name{j} = ['data-mmeasure-mse/dev/CH',num2str(j),'/autoencoder_scores.txt' ];
...
%        [names{j}, autoencoder_v{j}] = textread('autoencoder_scores.SimData_dt_for_1ch_far_room1_A.txt','%s %f ');
    ...
    ofdir = 'exp/dnn4_pretrain-dbn_dnn/decode_dev_mmeasure_autoencoder_comb/scoring';
    mkdir(ofdir);
    ofname = [ofdir,'/1.tra'];
