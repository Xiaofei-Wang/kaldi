...
if [ $stage -le -5 ]; then
    ...
    exp/dnn_pretrain-dbn_dnn/decode_dirha_sim_LA6 \
    ...
    exp/dnn_pretrain-dbn_dnn/decode_dirha_sim_KA6 \
    exp/dnn_pretrain-dbn_dnn/decode_sim_nobeam_badmic_score_combine || exit 1;

    local/score_combine.sh data-fmllr-tri4/dirha_real_LA6 $gmmdir/graph_tgpr_5k \
    exp/dnn_pretrain-dbn_dnn/decode_dirha_real_LA6 \
    ...
    exp/dnn_pretrain-dbn_dnn/decode_dirha_real_KA6 \
    exp/dnn_pretrain-dbn_dnn/decode_real_nobeam_badmic_score_combine || exit 1;
...
task=frame_tdnn_5layer_tdnn_11_11_nobeam_badmic
if [ $stage -le 1 ]; then
...
./local/pc/get_bashprocessing_nobeam_badmic.sh $task
./local/pc/postcombination_use_weight.sh $task

#./local/pc/get_bashprocessing_snr_nobeam_badmic.sh
#./local/pc/postcombination_use_weight.sh $task
