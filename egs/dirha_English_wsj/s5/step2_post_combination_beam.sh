...
if [ $stage -le -5 ]; then
    ...
    exp/dnn_pretrain-dbn_dnn/decode_dirha_sim_Beam_Linear_Array\
    exp/dnn_pretrain-dbn_dnn/decode_sim_beam_score_combine || exit 1;
    ...
    exp/dnn_pretrain-dbn_dnn/decode_dirha_real_Beam_Linear_Array\
    exp/dnn_pretrain-dbn_dnn/decode_real_beam_score_combine || exit 1;
fi
...
task=frame_tdnn_5layer_tdnn_8_5_beam
if [ $stage -le 1 ]; then
...
./local/pc/get_bashprocessing_beam.sh $task
./local/pc/postcombination_use_weight.sh $task

#./local/pc/get_bashprocessing_snr_beam.sh
#./local/pc/postcombination_use_weight.sh $task
