...
if [ $stage -le -5 ]; then
    ...
    exp/dnn_pretrain-dbn_dnn/decode_sim_nobeam_score_combine || exit 1;
    ...
    exp/dnn_pretrain-dbn_dnn/decode_real_nobeam_score_combine || exit 1;
...
task=frame_tdnn_5layer_tdnn_11_11_nobeam
if [ $stage -le 1 ]; then
...
./local/pc/get_bashprocessing.sh $task
./local/pc/postcombination_use_weight.sh $task

#./local/pc/get_bashprocessing_snr_nobeam.sh
#./local/pc/postcombination_use_weight.sh $task
