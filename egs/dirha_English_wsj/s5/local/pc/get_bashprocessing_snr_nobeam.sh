...
for i1 = 1: length(tasks)
 ...
 % process the utterance job-by-job
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_snr_nobeam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_snr_nobeam_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/snr_nobeam/postcombination_${JOB}.m
