...
mkdir -p local/pc/mmeasure_nobeam_badmic
...
for mm = 1: length(tasks)
  ...
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mmeasure_nobeam_badmic_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mmeasure_nobeam_badmic_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/mmeasure_nobeam_badmic/postcombination_${JOB}.m
