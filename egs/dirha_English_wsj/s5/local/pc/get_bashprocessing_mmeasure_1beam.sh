...
mkdir -p local/pc/mmeasure_1beam
...
for mm = 1: length(tasks)
  ...
  for job=$JOB
	...
	out_features.feature{j}=posterior2likelihood(feature{best_stream_idx}, prior_prob);           
	display(['sentence ',num2str(j),' (out of ',num2str(ark_utt_num),') finished!']);
    end


    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mmeasure_1beam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mmeasure_1beam_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/mmeasure_1beam/postcombination_${JOB}.m
