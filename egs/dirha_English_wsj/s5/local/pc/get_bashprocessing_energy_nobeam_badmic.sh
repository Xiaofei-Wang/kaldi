...
mkdir -p local/pc/energy_nobeam_badmic
...
for i1 = 1: length(tasks)
 ...
 % process the utterance job-by-job
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_energy_nobeam_badmic_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_energy_nobeam_badmic_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/energy_nobeam_badmic/postcombination_${JOB}.m
