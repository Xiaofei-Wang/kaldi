...
for i1 = 1: length(tasks)
 ...
 % process the utterance job-by-job
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_energy_1beam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_energy_1beam_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/energy_1beam/postcombination_${JOB}.m
