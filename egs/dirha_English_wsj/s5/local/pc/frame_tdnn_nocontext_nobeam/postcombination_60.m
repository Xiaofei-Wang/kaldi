...
for i1 = 1: length(scenes)
  ...
  scene=scenes{i1};
  weight_file=[weight_dir,scene,'_','frame_tdnn_nocontext_nobeam_comb/weight/stream_weights.scp'];
  ...
  for job=60 
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',scene,'_frame_tdnn_nocontext_nobeam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',scene,'_frame_tdnn_nocontext_nobeam_comb/post/post.',num2str(job),'.txt'];
