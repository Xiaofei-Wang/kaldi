...
#nnet_forward_opts="--no-softmax=true --prior-scale=0"
nnet_forward_opts="--apply-log=false --prior-scale=0"
...
if [ $stage -le 0 ]; then
  mkdir -p $dir/post
  $cmd --num-threads $((num_threads+1)) JOB=1:$nj $dir/post/log/decode.JOB.log \
    nnet-forward $nnet_forward_opts --feature-transform=$feature_transform --class-frame-counts=$class_frame_counts --use-gpu=$use_gpu "$nnet" "$feats" ark,t,scp:$dir/post/post.JOB.txt,$dir/post/post.JOB.scp || exit 1; 

  for ((n=1; n<=nj;n++));do
    cat $dir/post/post.${n}.scp >> $dir/post/feats.scp
  done
...
#    $model $graphdir/HCLG.fst ark:$dir/post_ori/post.JOB.txt "ark:|gzip -c > $dir/lat.JOB.gz" || exit 1;
#fi
