...
mkdir -p local/pc/snr_nobeam_badmic
  ...
  for job=$JOB
    ...
    for j = 1 : ark_utt_num
        ...
        for i2 = 1 : num_streams
		    ...
		    x_hat=MMSESTSA84(x,fs);
		    x_hat(end+1:end+length(x)-length(x_hat))=0;
                    buffer(i2) = snr(x,x_hat);
                    flag = 0;
                end
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_snr_nobeam_badmic_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_snr_nobeam_badmic_comb/post/post.',num2str(job),'.txt'];
...
" > local/pc/snr_nobeam_badmic/postcombination_${JOB}.m
