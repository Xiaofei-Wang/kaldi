...
mkdir -p local/pc/mdelta_nobeam
...
for mm = 1: length(tasks)
	...
	mic=mics{i1};
	mdelta_file=['data-mdelta/dirha_',task,'_',mic,'/data/mdelta_scores.dirha_',task,'_',mic,'.txt'];
	[names{i1}, mdelta_v{i1}] = textread(mdelta_file,'%s %f ');
    end

    num_utt=length(names{1});
    display('Finishing reading the mdelta file.');
  ...
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mdelta_nobeam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mdelta_nobeam_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   

  end
end
" > local/pc/mdelta_nobeam/postcombination_${JOB}.m
