...
for mm = 1: length(tasks)
  ...
  for job=$JOB
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mdelta_beam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_',task,'_mdelta_beam_comb/post/post.',num2str(job),'.txt'];
    writekaldifeatures(out_features, outfilename);   

  end
end
" > local/pc/mdelta_beam/postcombination_${JOB}.m
