...
%  task=tasks{i1};
  weight_file=[weight_dir,'sim_frame_entropy_nobeam_comb/weight/stream_weights.scp'];
  ...
  for job=98 
    ...
    for i2 = 1: num_streams
      ...
      mic=mics{i2};
      posterior_file=['exp/dnn_pretrain-dbn_dnn/decode_dirha_',mic,'_post/post/post.',num2str(job),'.scp'];
    ...
    outdir=['exp/dnn_pretrain-dbn_dnn/decode_sim_frame_entropy_nobeam_comb/post'];
    mkdir(outdir);
    outfilename=['exp/dnn_pretrain-dbn_dnn/decode_sim_frame_entropy_nobeam_comb/post/post.',num2str(job),'.txt'];
